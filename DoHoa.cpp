...
void n_box_ngang(int x, int y, int w, int h, int t_color, int b_color,int col)
{
	for (int i = 0;i < col;i++)
	{
		...
		if (i != 0)
		{
		 GoTo(x+(i*w),y);cout<<char(194);
		 GoTo(x+(i*w),y+h);cout<<char(193);
		}
	}
}
void n_box(int x, int y, int w, int h, int t_color, int b_color, int row)
{
	for (int i = 0;i < row;i++)
	{
		...
		if (i != 0)
		{
			GoTo(x, y + (i * 2));cout << char(195);
			GoTo(x + w, y + (i * 2));cout << char(180);
		}
	}
}
void bang(int x, int y, int w, int h, int t_color, int b_color, int col, int row)
{
	for(int i=0;i<row;i++)
	{
		...
		if (i != 0)
		{   
			GoTo(x, y + (i * h));cout << char(195);
			GoTo(x + w, y + (i * h));cout << char(180);
		}		
	}	
}
...
void menu()
{
	string a[4]={"1.Xem Hom Nay Co Phim Gi:","2.Tim Kiem Phim","3.Tim Kiem Phim Theo The Loai","4.Doi Mat Khau"};
	ShowCur(0);
	//----- setting ----
	...
	//-------------
	int xp = x;int yp = y;//toa do thanh sang
